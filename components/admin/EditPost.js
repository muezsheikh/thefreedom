...
const [oldImg, setOldImg] = useState(true)
  ...
  const deleteImage = async (imageUrls) => {
    try {
      ...
      const deletingToast = toast.info('Deleting image...', { autoClose: false });
  ...
  const getCategories = async () => {
    try {
      const response = await axios.get(`${process.env.NEXT_PUBLIC_HOST}/api/categories/get`);
  ...
  const updateData = async () => {
    try {
      ...
      const { data } = await axios.put(`${process.env.NEXT_PUBLIC_HOST}/api/posts/updatepost/${postIds}`, postDetails);
          ...
          <div className='newPostContent'>
            ...
            <div className='inputGroup'>
              ...
              <div className="uploadedImg">
                <img style={{width: '200px'}} className='uploadText' src={postContent.uploadImg} />
                <button><i onClick={()=>deleteImage(postContent.uploadImg)} className="fas fa-xmark"></i></button>
              </div>
              }
              {imageUrl && 
              <div className="uploadedImg">
                <img style={{width: '200px'}} className='uploadText' src={imageUrl} />
                <button><i onClick={()=>deleteImage(imageUrl)} className="fas fa-xmark"></i></button>
              </div>
              }
            </div>
            <div className='inputGroup inputGroupB'>
              <label>Banner</label>
              <input  type='checkbox' checked={postContent.banner} value={postContent.banner} onChange={(e) => postContentHandler('banner', e.target.checked)} />
              
            </div>
            <div className='inputGroup'>
             <div className='categories'>
                <label>Choose Category:</label>
                <select value={postContent.sCategory} onChange={(e) => postContentHandler('sCategory', e.target.value)}>
                  <option value={null} disabled>Choose Category</option>
                  {categories && categories?.categories?.map((cat) => (
                    <option key={cat._id} value={cat.link}>{cat.category}</option>
                  ))}
                </select>
              </div>
              <div className='inputGroup'>
                ...
                />
                <div style={{ display: 'flex', gap: '10px' }} className='tagsContainer'>
            ...
            <div className='submitButton'>
              ...
              <button onClick={cancelUpdate} style={{marginLeft: '10px', backgroundColor: 'red'}}>Cancel Update</button>
            </div>
          </div>
        </div>
      </div>
